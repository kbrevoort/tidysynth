...
prepare_data <- function(data, i, t, treated_def, preperiod, y, ...) {
  ...
  if (!all(c(as.character(i_var), as.character(t_var)) %in% names(data)))
  ...
  treated_data <- define_treatment(data, enquo(treated_def))
...
define_treatment <- function(data, i_name, treated_def) {
  treated_flag <- quo_name(treated_def) %>%
    substitute() %>%
